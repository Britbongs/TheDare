...
void MTileMap::setupLayer(xml_node<>* mapNode)
{
	...
	while (layerNode != nullptr)
	{
		...
		vector<int> a(width_); //a temporary vector we'll use as an example, will be pushed into a vector
...
void MTileMap::setupObjectGroups(xml_node<>* mapNode)
{
	...
	xml_node<>* objectPropertyNode(nullptr); // Pointer to access the object's properties
